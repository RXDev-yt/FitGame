...
<style>
  ...
  *{ box-sizing:border-box; margin:0; padding:0; -webkit-tap-highlight-color:transparent; }
  ...
  .seclbl{ font-size:11px; font-weight:900; letter-spacing:.14em; text-transform:uppercase; color:var(--dim); margin:18px 0 9px; }
  ...
  .dr-enemy{ display:flex; gap:14px; align-items:center; margin-bottom:14px; }
  ...
  .enemy-line{ display:flex; align-items:center; gap:12px; }
  .enemy-sprite{ font-size:40px; animation:bob 2s infinite; filter:drop-shadow(0 4px 6px rgba(0,0,0,.6)); }
  .combat-hud.hit .enemy-sprite{ animation:shake .22s ease; }
  ...
  .enemy-sprite.enter{ animation:enemyEnter .5s cubic-bezier(.2,.9,.3,1.4); }
  @keyframes enemyEnter{ from{ opacity:0; transform:translateY(-26px) scale(.5) rotate(-12deg);} }
  ...
  #loader{ position:absolute; inset:0; z-index:200; display:flex; flex-direction:column; align-items:center;
    justify-content:center; gap:16px; background:radial-gradient(circle at 50% 35%,#241458,#07040F);
    transition:opacity .5s ease, visibility .5s; }
  #loader.gone{ opacity:0; visibility:hidden; }
  .ld-logo{ width:104px; height:104px; border-radius:30px; background:var(--grad-purple);
    display:flex; align-items:center; justify-content:center; position:relative;
    box-shadow:0 0 60px rgba(139,92,246,.7); animation:ldPulse 1.6s ease-in-out infinite; }
  @keyframes ldPulse{ 50%{ transform:scale(1.08) rotate(-3deg); box-shadow:0 0 90px rgba(245,158,11,.7);} }
  .ld-ring{ position:absolute; inset:-14px; border-radius:38px; border:2px solid rgba(245,158,11,.5);
    animation:ldRing 1.8s ease-out infinite; }
  @keyframes ldRing{ 0%{ transform:scale(.9); opacity:.9;} 100%{ transform:scale(1.35); opacity:0;} }
  .ld-title{ font-size:30px; font-weight:900; letter-spacing:.02em; text-shadow:0 4px 0 rgba(0,0,0,.5),0 0 30px rgba(139,92,246,.6); }
  .ld-tip{ font-size:12px; font-weight:800; color:var(--dim); max-width:26ch; text-align:center; min-height:34px;
    animation:tipFade .5s ease; }
  @keyframes tipFade{ from{ opacity:0; transform:translateY(6px);} }
  .ld-track{ width:200px; height:7px; border-radius:99px; background:rgba(255,255,255,.10); overflow:hidden; }
  .ld-track i{ display:block; height:100%; width:0%; border-radius:99px;
    background:linear-gradient(90deg,var(--violet-light),var(--gold)); transition:width .4s ease;
    box-shadow:0 0 14px rgba(245,158,11,.7); }
  .ld-pct{ font-size:11px; font-weight:900; color:var(--gold); letter-spacing:.14em; }
  ...
  .hex.frame.enemy{ background:var(--red); }
...
<section class="screen active" id="scr-home">
  <div class="statusbar">
    <div class="av-wrap" id="home-av">
      <span class="lvl-badge">LVL <span id="h-lvl">5</span></span>
      <div class="avatar">YOU</div>
      <span class="av-pet" id="h-pet"></span>
    </div>
    <div class="xp-mini">
      <div class="t"><span>XP</span><span><span id="h-xp">90</span>/<span id="h-xpn">250</span></span></div>
      <div class="pbar fill-p"><i id="h-xpbar" style="width:36%"></i></div>
    </div>
    <div class="pillstat" id="ps-flame"><span id="h-streak">7</span></div>
    <div class="pillstat" id="ps-coin"><span id="h-coins">1,450</span></div>
  </div>

  <div class="card boss-banner">
    <div class="row" style="gap:14px">
      <div class="boss-sprite float-slow" id="db-sprite"></div>
      <div class="grow">
        <div class="boss-tag">TODAY'S BOSS</div>
        <div class="boss-nm hdr glow" id="db-name">Goblin Chief</div>
        <div class="hp-line">
          <div class="t"><span class="red">HP</span><span id="db-hp">60 / 60</span></div>
          <div class="pbar fill-r"><i id="db-bar" style="width:100%"></i></div>
        </div>
        <div class="reward-chips">
          <span class="rchip"><span id="db-target">60</span> reps to defeat</span>
          <span class="rchip gold">+150 XP</span>
          <span class="rchip gold" id="db-coinchip">200</span>
        </div>
      </div>
    </div>
  </div>

  <button class="btn b-cta hdr" style="font-size:17px;padding:18px" onclick="fightDailyBoss()">⚔️ START WORKOUT</button>

  <div class="seclbl">Daily Quests</div>
  <div id="quest-list"></div>

  <div class="seclbl">Today</div>
  <div class="qstats">
    <div class="qstat"><div class="n" id="qs-reps">0</div><div class="l">Reps</div></div>
    <div class="qstat"><div class="n" id="qs-time">0:00</div><div class="l">Active</div></div>
    <div class="qstat"><div class="n" id="qs-cal">0</div><div class="l">Kcal</div></div>
  </div>
</section>
  ...
  <div class="card chap-head">
    <div class="row" style="gap:12px">
      ...
      <div class="grow">
        ...
        <div class="hdr" style="font-size:19px" id="ch-name">Goblin Caves</div>
...
<section class="screen" id="scr-arena">
  <div class="topline"><div class="screen-title hdr">Duels</div><span class="chip hot" id="duel-elo">1000 ELO</span></div>

  <div class="rank-card" id="rankcard" style="--rc1:#8C5A2B;--rc2:#5B3617;margin-bottom:14px">
    <div class="rank-emblem" id="rank-emblem"></div>
    <div class="rank-tier" id="rank-tier">BRONZE</div>
    <div class="rank-sub"><span id="rank-elo-n">1000</span> ELO</div>
    <div class="rank-prog">
  ...
  <div class="card mode-card" style="border-color:rgba(245,158,11,.45);margin-top:14px">
    <div class="row" style="gap:13px">
      <svg viewBox="0 0 24 24" style="width:28px;height:28px" fill="none" stroke="#F59E0B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3l8 8M3 3v4M3 3h4M21 3l-8 8M21 3v4M21 3h-4M7 17l-3 3M17 17l3 3M6 14l4 4M18 14l-4 4"/></svg>
      <div class="grow"><b>Ranked 1v1</b><p>Live rep race. Win to climb the ELO ladder.</p></div>
    </div>
    <button class="btn b-gold" style="margin-top:12px" onclick="findMatch()">FIND RANKED MATCH</button>
  </div>

  <div class="card mode-card">
    <div class="row" style="gap:13px">
      <svg viewBox="0 0 24 24" style="width:28px;height:28px" fill="none" stroke="#8B5CF6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="8" r="3.4"/><path d="M2.5 20c.8-3.2 3.4-5 6.5-5s5.7 1.8 6.5 5"/><circle cx="17.5" cy="9" r="2.6"/><path d="M15.7 14.4c2.8.2 5 1.9 5.8 4.6"/></svg>
      <div class="grow"><b>Fight a friend</b><p>Create a room, send them the code.</p></div>
    </div>
    <button class="btn b-purple" style="margin-top:12px" onclick="openLobby()">PRIVATE DUEL</button>
  </div>

  <div class="seclbl">Your record</div>
  <div class="today-grid">
    <div class="mini-stat"><div class="n gold" id="duel-elo2">1000</div><div class="l">ELO rating</div></div>
    <div class="mini-stat"><div class="n green" id="duel-wins">0</div><div class="l">Duel wins</div></div>
  </div>
</section>
...
<section class="screen" id="scr-battle">
  <div class="combat-hud" id="c-hud">
    <div class="enemy-line">
      <span class="enemy-sprite" id="b-sprite"></span>
      <div>
    ...
    <div class="hp-line">
      <div class="t"><span class="red" id="b-hplbl">HP</span><span id="b-hptxt">3 / 5</span></div>
      <div class="pbar fill-r"><i id="b-hpbar" style="width:60%"></i></div>
    </div>
    <div class="php-wrap">
      <div class="php-lbl"><span class="green">YOUR STAMINA</span><span id="php-txt">100 / 100</span></div>
...
<section class="screen" id="scr-duel">
    ...
    <div class="vs-intro gone" id="vs-intro">
      <div class="vs-card">
        ...
        <div class="vs-fighter right">
          <div class="hexwrap"><div class="hex frame enemy"></div><div class="hex inner" id="vs-op-av"></div></div>
  ...
  <div class="row" style="gap:10px;margin-top:10px">
    <button class="btn b-ghost" style="flex:1" onclick="abortDuel()">QUIT</button>
...
<section class="screen" id="scr-profile">
  <div class="topline">
    <div class="screen-title hdr">Account</div>
    <button class="gear-btn" onclick="profTab('set')" aria-label="Settings">
  ...
  <div id="p-char">
    <div class="prof-hero">
      ...
      <div class="hdr" style="font-size:21px" id="prof-uname">Fighter</div>
      <span class="prof-title" id="p-title">ROOKIE</span>
      <div class="rank-elo" style="margin-top:8px;font-size:13px"><span id="p-elo">1000</span> ELO · LVL <span id="p-lvl">1</span></div>
    </div>

    <div class="seclbl">Bio</div>
    ...
    <div class="seclbl">Badges</div>
    <div class="badge-grid" id="badge-grid"></div>

    <div class="seclbl">Profile Icon</div>
    <div class="av-grid" id="av-grid"></div>
    <p class="dim" style="font-size:11px;font-weight:800;margin-top:8px">Uploading a photo is optional — your icon works fine on its own.</p>

    <div class="seclbl">Frames</div>
    <div id="shop-frames"></div>
    <div class="seclbl">Titles</div>
    <div id="shop-titles"></div>
    <div class="seclbl">Combat Pets</div>
    <div id="shop-pets"></div>
  </div>

  <div id="p-ana" style="display:none">
    <div class="today-grid">
      <div class="mini-stat"><div class="n" id="p-total">0</div><div class="l">Total reps</div></div>
      <div class="mini-stat"><div class="n gold" id="p-streak">0</div><div class="l">Day streak</div></div>
    </div>
    <div class="card"><b style="font-size:13.5px">Weekly rep volume</b><div class="chart" id="ana-chart"></div></div>
    <div class="card"><b style="font-size:13.5px">Exercise distribution</b><div style="margin-top:11px" id="ana-dist"></div></div>
    <div class="card"><b style="font-size:13.5px">Streak heatmap — last 4 weeks</b><div class="heat" id="ana-heat" style="margin-top:11px"></div></div>
  </div>

  <div id="p-set" style="display:none">
    <div class="set-row">🔊 Sound FX <button class="toggle on" id="set-sound" onclick="toggleSound()"><i></i></button></div>
    <div class="set-row">🎯 Pose sensitivity
      <div class="sens">
        <button onclick="setSens(1)" id="sn1">1</button>
        <button class="on" onclick="setSens(2)" id="sn2">2</button>
        <button onclick="setSens(3)" id="sn3">3</button>
      </div>
    </div>
    <div class="set-row">
      <svg viewBox="0 0 24 24" style="width:19px;height:19px;flex:none" fill="none" stroke="#F59E0B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.7 6.3a4 4 0 0 0-5.4 5.4L3 18v3h3l6.3-6.3a4 4 0 0 0 5.4-5.4L15 12l-3-3z"/></svg>
      Test camera on phone
      <button class="claim" style="margin-left:auto" onclick="openDevModal()">GUIDE</button>
    </div>
    <div class="set-row">👤 Account <span class="dim" style="margin-left:auto;font-size:12px" id="set-email">—</span></div>
    <div class="set-row">⭐ Membership <span class="own" style="margin-left:auto">FOUNDER ✓</span></div>
    <div class="set-row">🚪 Log out <button class="claim" style="margin-left:auto;background:var(--red);color:#fff" onclick="confirmLogout()">SIGN OUT</button></div>
  </div>
</section>
...
<section class="screen fullflow active" id="scr-splash">
  <div class="splash">
    <div class="logo-big">
      <svg viewBox="0 0 24 24" style="width:48px;height:48px" fill="#fff"><path d="M13 2 4 14h6l-1 8 9-12h-6l1-8z"/></svg>
    </div>
    <h1 class="hdr">RepQuest</h1>
    <p>Train · Fight · Level Up</p>
  </div>
</section>
...
<section class="screen fullflow" id="scr-auth">
  <div class="authbox">
    <div class="logo-big">
      <svg viewBox="0 0 24 24" style="width:32px;height:32px" fill="#fff"><path d="M13 2 4 14h6l-1 8 9-12h-6l1-8z"/></svg>
    </div>
    ...
    <div class="field" id="f-username" style="display:none">
      <label>Username</label>
      <input id="in-username" type="text" placeholder="IronPusher_CZ" maxlength="20" autocomplete="username">
    </div>
    <div class="field">
      <label>Email</label>
  ...
  <div class="obwrap">
    ...
    <div class="ob-step on" id="ob1">
      <div class="ob-hero">
        ...
        <div class="ob-title hdr">Every hero needs a name</div>
        <div class="ob-sub">This is the name other fighters see on the leaderboard and in duels.</div>
      </div>
      <div class="field">
        <label>Your fighter name</label>
      ...
      <div class="row" style="gap:12px;align-items:center;margin-bottom:12px">
        ...
        <span class="dim" style="font-size:11.5px;font-weight:800">A monster icon is picked already — uploading a photo is optional</span>
      </div>
    ...
    <div class="ob-step" id="ob2">
      <div class="ob-hero">
        ...
        <div class="ob-title hdr glow" id="ob-greet">The Arena Calls</div>
        <div class="ob-sub" id="ob-story">The old world had gyms. Ours has monsters.<br><br>
          When the Hollow War ended the gates opened, and the strong were the only ones left standing.
          You are no one yet — no rank, no name they fear. Just arms that still work.</div>
      </div>
      <div class="taste-row">
        <div class="taste-ico" id="ti1"></div>
        <div><b>Your reps are your weapon</b><span>Every clean rep is a strike. Combos hit harder.</span></div>
      </div>
      <div class="taste-row">
      ...
      <div class="taste-row">
        ...
        <div><b>108 episodes, 6 chapters</b><span>Goblin Caves → the crown at Dragons Peak</span></div>
      </div>
      <div class="taste-row">
    ...
    <div class="ob-step" id="ob3">
      <div class="ob-hero">
        ...
        <div class="ob-sub">RepQuest counts your reps with the camera. Nothing is recorded, uploaded, or saved — the video never leaves your phone.</div>
      </div>
...
<nav id="nav">
  <button class="nv on" data-s="scr-home" onclick="nav('scr-home')">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v4M5 8h14l1 12H4z"/><path d="M9 13h6"/></svg>Quests</button>
  ...
  <button class="nv" data-s="scr-arena" onclick="nav('scr-arena')">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3l8 8M3 3v4M3 3h4M21 3l-8 8M21 3v4M21 3h-4M7 17l-3 3M17 17l3 3M6 14l4 4M18 14l-4 4"/></svg>Duels</button>
  <button class="nv" data-s="scr-profile" onclick="nav('scr-profile')">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 21c1-4 4-6 8-6s7 2 8 6"/></svg>Account</button>
  <button class="nv lock" data-s="scr-inventory" onclick="nav('scr-inventory')">
...
<div id="loader">
  <div class="ld-logo">
    ...
    <svg viewBox="0 0 24 24" style="width:52px;height:52px" fill="#fff"><path d="M13 2 4 14h6l-1 8 9-12h-6l1-8z"/></svg>
  </div>
  <div class="ld-title hdr">RepQuest</div>
  <div class="ld-tip" id="ld-tip">Waking the arena…</div>
...
const S={
  ...
  dailyBoss:{ name:'Goblin Chief', sprite:'chief', hp:60, max:60, done:false, rwXP:150, rwCoins:200 },
  ...
  shop:{
    ...
    titles:[ {id:'ROOKIE',ico:()=>ICO.medal(22,'#94A3B8'),name:'Rookie',cost:0,own:true},
             ...
             {id:'BOSS SLAYER',ico:()=>ICO.skull(22),name:'Boss Slayer',cost:500,own:false} ],
...
const ART={
  ...
  spider:`<ellipse cx="32" cy="38" rx="14" ry="12" fill="#1E1B34"/><circle cx="32" cy="24" r="8" fill="#2D2A4A"/><circle cx="29" cy="23" r="2.2" fill="#EF4444"/><circle cx="35" cy="23" r="2.2" fill="#EF4444"/><circle cx="32" cy="27.5" r="1.4" fill="#EF4444"/><g stroke="#2D2A4A" stroke-width="3" stroke-linecap="round" fill="none"><path d="M20 32 Q8 26 6 16"/><path d="M19 38 Q6 38 4 30"/><path d="M20 44 Q10 50 8 58"/><path d="M44 32 Q56 26 58 16"/><path d="M45 38 Q58 38 60 30"/><path d="M44 44 Q54 50 56 58"/></g><path d="M26 40 q6 4 12 0" stroke="#8B5CF6" stroke-width="2" fill="none"/>`,
  bone:`<circle cx="32" cy="30" r="16" fill="#E2E8F0"/><rect x="24" y="42" width="16" height="8" rx="3" fill="#E2E8F0"/><ellipse cx="26" cy="29" rx="4.4" ry="5.2" fill="#0F172A"/><ellipse cx="38" cy="29" rx="4.4" ry="5.2" fill="#0F172A"/><circle cx="27" cy="28" r="1.4" fill="#F59E0B"/><circle cx="39" cy="28" r="1.4" fill="#F59E0B"/><path d="M29 39 h6" stroke="#94A3B8" stroke-width="2"/><path d="M28 44 v4 M32 44 v4 M36 44 v4" stroke="#94A3B8" stroke-width="2"/><path d="M30 20 l2 -4 2 4" fill="none" stroke="#CBD5E1" stroke-width="2"/>`,
  ...
  warlord:`<ellipse cx="32" cy="38" rx="20" ry="17" fill="#4ADE80"/><path d="M12 32 L4 22 L16 27 Z" fill="#4ADE80"/><path d="M52 32 L60 22 L48 27 Z" fill="#4ADE80"/><path d="M14 28 Q14 10 32 8 Q50 10 50 28 L44 24 Q32 18 20 24 Z" fill="#64748B"/><path d="M28 8 l4 -6 4 6 Z" fill="#EF4444"/><circle cx="25" cy="37" r="3.8" fill="#0F172A"/><circle cx="39" cy="37" r="3.8" fill="#0F172A"/><circle cx="26.2" cy="36" r="1.2" fill="#EF4444"/><circle cx="40.2" cy="36" r="1.2" fill="#EF4444"/><path d="M22 48 Q32 44 42 48" stroke="#0F172A" stroke-width="3" fill="none" stroke-linecap="round"/><path d="M25 48 l0 -5 4 5 Z" fill="#fff"/><path d="M39 48 l0 -5 -4 5 Z" fill="#fff"/>`,
  skelking:`<circle cx="32" cy="32" r="15" fill="#E2E8F0"/><path d="M18 22 L18 10 L24 17 L32 8 L40 17 L46 10 L46 22 Z" fill="#F59E0B"/><circle cx="21" cy="12" r="2" fill="#EF4444"/><circle cx="32" cy="10" r="2" fill="#EF4444"/><circle cx="43" cy="12" r="2" fill="#EF4444"/><ellipse cx="26" cy="31" rx="4.2" ry="5" fill="#0F172A"/><ellipse cx="38" cy="31" rx="4.2" ry="5" fill="#0F172A"/><circle cx="27" cy="30" r="1.4" fill="#8B5CF6"/><circle cx="39" cy="30" r="1.4" fill="#8B5CF6"/><path d="M27 41 h10 M28 41 v4 M32 41 v4 M36 41 v4" stroke="#94A3B8" stroke-width="2"/>`,
...
const ICO={
  ...
  skull:s=>`<svg viewBox="0 0 24 24" style="width:${s}px;height:${s}px" fill="#E2E8F0"><circle cx="12" cy="10" r="8"/><rect x="8" y="16" width="8" height="4" rx="1.4"/><ellipse cx="9" cy="10" rx="2" ry="2.4" fill="#0F172A"/><ellipse cx="15" cy="10" rx="2" ry="2.4" fill="#0F172A"/><path d="M10.5 20v-2M13.5 20v-2" stroke="#94A3B8" stroke-width="1.4"/></svg>`,
...
const CHAPTERS=[
  { name:'Goblin Caves', icon:'goblin',
    pool:[['Goblin','goblin'],['Cave Bat','bat'],['Goblin Archer','archer'],['Cave Spider','spider'],['Bone Grunt','bone'],['Troll','troll']],
    boss:['Goblin Warlord','warlord'] },
  { name:'Crypt of Bones', icon:'skelking',
    pool:[['Skeleton','bone'],['Ghoul','ghoul'],['Crypt Bat','bat'],['Grave Spider','spider'],['Wraith','wraith'],['Bone Troll','troll']],
    boss:['Skeleton King','skelking'] },
  { name:'Spiderfang Hollow', icon:'brood',
    pool:[['Spiderling','spider'],['Venom Bat','bat'],['Web Ghoul','ghoul'],['Fang Goblin','goblin'],['Shadow Wraith','wraith'],['Cave Troll','troll']],
    boss:['Broodmother','brood'] },
  { name:'Ashfang Keep', icon:'warlord',
    pool:[['Ash Goblin','goblin'],['Ember Bat','bat'],['Cinder Ghoul','ghoul'],['Forge Troll','troll'],['Ash Archer','archer'],['Molten Bones','bone']],
    boss:['Ashfang Warlord','warlord'] },
  { name:'Frostbound Depths', icon:'wraith',
    pool:[['Frost Wraith','wraith'],['Ice Spider','spider'],['Rime Ghoul','ghoul'],['Glacier Troll','troll'],['Frozen Bones','bone'],['Snow Bat','bat']],
    boss:['Hollow King','skelking'] },
  { name:'Dragons Peak', icon:'dragonet',
    pool:[['Drake Whelp','dragonet'],['Peak Wraith','wraith'],['Stone Troll','troll'],['Wyrm Spider','spider'],['Bone Drake','bone'],['Sky Archer','archer']],
    boss:['Elder Dragon','dragonet'] },
];
...
CHAPTERS.forEach((c,ci)=>{
  for(let i=0;i<18;i++){
    ...
    S.chapters[ci].eps.push({
      ...
      stars: (ci===0&&i===0)?2:0,
      exs: [['Push-ups'],['Sit-ups'],['Push-ups'],['Squats']][i%4],
      drops: boss?'Chapter Crown · 80–120 coins':'Small HP Potion · '+(5+g)+'–'+(15+g)+' coins',
    });
  }
});
...
/* ================= HOME ================= */
function renderHome(){
  if(!$('ps-flame').dataset.ico){ $('ps-flame').insertAdjacentHTML('afterbegin',ICO.flame(15)); $('ps-flame').dataset.ico=1;
    $('ps-coin').insertAdjacentHTML('afterbegin',ICO.coin(15)); $('ps-coin').dataset.ico=1; }
  if(PROFILE){ $('home-uname').textContent=PROFILE.username; paintAvatars(); }
  if($('diff-card-home')) $('diff-card-home').innerHTML=diffPickerHTML();
  const cch=S.chapters[S.cur.c];
  if($('camp-ico')){ $('camp-ico').innerHTML=mon(cch.icon,26);
    $('camp-name').textContent=cch.name;
    $('camp-sub').textContent='Chapter '+(S.cur.c+1)+' · Episode '+(S.cur.e+1)+'/18'; }
  $('h-lvl').textContent=S.user.lvl;
  $('h-xp').textContent=S.user.xp; $('h-xpn').textContent=xpNeed();
  const xb=$('h-xpbar'); xb.style.width=(S.user.xp/xpNeed()*100)+'%';
  xb.classList.remove('xpbar-glow'); void xb.offsetWidth; xb.classList.add('xpbar-glow');
  $('h-streak').textContent=S.user.streak;
  $('h-coins').textContent=S.user.coins.toLocaleString('en-US');
  $('h-pet').innerHTML=S.user.pet?mon(S.user.pet,20):'';
  applyFrame($('home-av'));
  const b=S.dailyBoss;
  $('db-name').textContent=b.done?b.name+' ✓ DEFEATED':b.name;
...
function renderMap(){
  ...
  $('ch-num').textContent='CHAPTER '+(S.viewC+1)+' / '+S.chapters.length;
  ...
  if($('ch-global')) $('ch-global').textContent=allDone+' / '+allEps+' episodes cleared';
  $('ch-icon').innerHTML=mon(ch.icon,40);
  $('ch-prog').textContent=unlocked?Math.min(done+1,18):0;
  $('ch-stars').textContent=ch.eps.reduce((a,e)=>a+e.stars,0);
  $('ch-prev').disabled=S.viewC===0;
  $('ch-next').disabled=S.viewC===S.chapters.length-1;
  $('ch-lock').style.display=unlocked?'none':'block';
  const wrap=$('map-nodes'), svg=$('map-svg'), W=$('map-wrap').clientWidth||360;
  const H=18*94+40; $('map-wrap').style.height=H+'px';
...
function openDrawer(ci,i){
  ...
  dr.innerHTML=`
    ...
    <div class="dr-enemy">
      ...
      <div class="grow">
        <div class="boss-tag">${e.boss?'BOSS EPISODE':'EPISODE '+(i+1)}</div>
        <div class="hdr" style="font-size:18px">${e.name}</div>
        <div class="dim" style="font-size:11.5px;font-weight:800">${e.hp} HP ${e.waves>1?'· '+e.waves+' waves':''} ${e.stars?'· '+'★'.repeat(e.stars):''}</div>
      </div>
...
function makeBattle(cfg){
  S.B={ ...cfg, waveIdx:0, reps:0, sec:0, paused:false, timers:[], repTs:[], eHP:cfg.waves[0].hp,
        tracking:false, live:false, php:100, lastRepAt:0, gate:true };
  goFlow('scr-battle');
  if($('b-diff')) $('b-diff').textContent=D().name.toUpperCase()+' · '+cfg.exercise.toUpperCase();
  ...
  S.B.timers.push(t);
  const atk=setInterval(enemyStrike, (ENEMY_ATK[curDiff()]||ENEMY_ATK.normal).ms);
  S.B.timers.push(atk);
}
async function startBattleTracking(){
  ...
  if(!camGranted){
    try{ const s=await navigator.mediaDevices.getUserMedia({video:{facingMode:'user'}}); s.getTracks().forEach(t=>t.stop()); camGranted=true; }
      ...
      openModal('<div class="em">📷</div><h3 class="hdr">Camera needed</h3><p>RepQuest counts reps with the camera. Allow camera access for this site, then try again.</p><button class="btn b-purple" onclick="closeModal();quitBattle()">OK</button>');
      return;
    }
  }
  const ok=await PoseEngine.start('b-video','b-canvas',myB.exercise,
    ...
    (type,msg)=>{
      ...
      if(S.B===myB && S.B.gate===true && typeof msg==='string' && msg.indexOf('IN POSITION')>-1) onInPosition();
    });
...
/* enemy damage per strike + how often it swings (ms) */
...
function startRest(nextName,nextSprite){
  ...
  $('rest-next').textContent=nextName||'Next enemy';
...
function endRest(){
  ...
  B.gate=true;
  showGate(true,'Next wave','Get back into position — the next enemy is coming.');
  SFX.kill();
}
...
function enemyStrike(){
  const B=S.B; if(!B||B.gate||B.resting||B.paused||B.noHP) return;
...
function tickStamina(){
  const B=S.B; if(!B||B.gate||B.noHP) return;
  const idle=(performance.now()-B.lastRepAt)/1000;
  if(idle>4){
    B.php-=D().drain;                              /* stalling drains you */
    if(B.php<=30 && !B._warned){ B._warned=true; toast('Stamina draining — keep repping!'); SFX.hurt(); }
    syncStamina();
    if(B.php<=0){ B.php=0; syncStamina(); loseBattle(); }
  }
}
function loseBattle(){
  ...
  vicCtx={ origin:S.B_origin, headline:'DEFEATED', stars:0, reps, sec, coins:reps*1, win:false };
...
function syncEnemyHUD(spawn){
  ...
  $('b-hplbl').textContent=B.noHP?'PR TARGET':'HP';
  $('b-hptxt').textContent=B.noHP?(B.reps+' / '+S.user.pr):(B.eHP+' / '+w.hp);
  $('b-hpbar').style.width=B.noHP?Math.min(100,B.reps/S.user.pr*100)+'%':(B.eHP/w.hp*100)+'%';
  if(spawn){
...
function battleRep(){
  ...
  if(!B.noHP){
    ...
    const d=document.createElement('div'); d.className='dmg'+(crit?' crit':''); d.textContent=crit?'CRIT -3!':'-1 HP!';
    ...
    if(B.eHP<=0){
      ...
      if(B.infinite){
        B.waveIdx++; B.waves.push(nextHordeWave(B.waveIdx)); B.eHP=B.waves[B.waveIdx].hp;
        syncEnemyHUD(true); startRest(B.waves[B.waveIdx].name,B.waves[B.waveIdx].sprite); return;
      }
      else if(B.waveIdx<B.waves.length-1){
        B.waveIdx++; B.eHP=B.waves[B.waveIdx].hp;
        syncEnemyHUD(true); startRest(B.waves[B.waveIdx].name,B.waves[B.waveIdx].sprite); return;
      }
...
function pauseBattle(){
  const B=S.B; if(!B) return;
  B.paused=true;
  openModal(`<div class="em">⏸</div><h3 class="hdr">Paused</h3><p>Catch your breath, warrior.</p>
    <button class="btn b-purple" onclick="resumeBattle()">RESUME</button>
    <button class="btn b-ghost" onclick="quitBattle()">QUIT (no rewards)</button>`);
}
...
function killBattle(){ if(!S.B) return; $('restgate')?.classList.remove('on'); S.B.timers.forEach(t=>{clearInterval(t);clearTimeout(t);}); S.B._dead=true; S.B=null; }
...
function fightDailyBoss(){
  const b=S.dailyBoss; S.B_origin='home';
...
function startSpeedRun(){ S.B_origin='arena';
  makeBattle({ kind:'speed', exercise:S.speedEx, target:S.speedTarget, waves:[{name:'Training Dummy',sprite:'🎯',hp:S.speedTarget}], modeName:'Speed Run' }); }
...
function startMax(){ S.B_origin='arena';
  makeBattle({ kind:'max', exercise:S.speedEx, noHP:true, endset:true, waves:[{name:'Training Dummy',sprite:'🎯',hp:1}], modeName:'Max Reps' }); }
...
function finishBattle(win){
  ...
  if(B.kind==='speed'){ const pace=sec/Math.max(1,reps); stars=pace<2?3:pace<3?2:1; bonusC=30;
    ...
    stars=3; headline='FIRST BLOOD!'; bonusXP=50; bonusC=50;
...
/* ================= REAL STATS TRACKING ================= */
function statsKey(){ return 'rq_stats_'+(ME?.id||'anon'); }
function loadStats(){
...
const STORY={
  intro:{
    ...
    lines:[
      'The old world had gyms. Ours has monsters.',
      'When the Hollow War ended, the gates opened — and the strong were the only ones left standing.',
      'You are no one yet. No rank, no name they fear. Just arms that still work.',
      'Every rep you complete is a strike. Every set, a battle. The arena does not care how you feel.',
      'Cut through the caves. Take the crown at Dragons Peak.',
    ],
  },
  chapters:[
    { t:'Chapter I — Goblin Caves',      s:'They breed in the dark under the old training grounds. Small, fast, endless. Everyone starts here. Most never leave.' },
    { t:'Chapter II — Crypt of Bones',   s:'The dead here trained too, once. Their discipline outlived their flesh. Now they test yours.' },
    { t:'Chapter III — Spiderfang Hollow',s:'Webs thick as rope. The Broodmother has been counting your reps since you entered. She is patient.' },
    { t:'Chapter IV — Ashfang Keep',     s:'A fortress built by warriors who lifted mountains. Its forge still burns. Its guards still remember how to break arms.' },
    { t:'Chapter V — Frostbound Depths', s:'Cold enough to seize a muscle mid-rep. The Hollow King rules here — he was a champion once too, before he stopped moving.' },
    { t:'Chapter VI — Dragons Peak',     s:'Above the clouds. Thin air, heavy limbs, and the Elder Dragon waiting at the summit. This is what all of it was for.' },
  ],
};
function showStoryModal(title,body,btn,cb){
  openModal('<div class="em">📜</div><h3 class="hdr">'+title+'</h3><p style="line-height:1.7">'+body+'</p>'+
...
/* ================= DIFFICULTY ================= */
const DIFFS={
  easy:      { name:'Easy',      hp:0.7, xp:0.7, coin:0.7, drain:2.0, col:'#10B981', desc:'Fewer reps, gentler stamina drain' },
  normal:    { name:'Normal',    hp:1.0, xp:1.0, coin:1.0, drain:3.5, col:'#8B5CF6', desc:'The intended fight' },
  hard:      { name:'Hard',      hp:1.4, xp:1.5, coin:1.5, drain:5.0, col:'#F59E0B', desc:'Tougher enemies, better rewards' },
  nightmare: { name:'Nightmare', hp:2.0, xp:2.2, coin:2.2, drain:7.0, col:'#EF4444', desc:'Brutal. Huge XP and coins.' },
};
  ...
  rd.onload=()=>{
    ...
    img.onload=()=>{
      ...
      try{
        ...
        console.error('[RepQuest] avatar processing failed', e);
        if(btn) btn.classList.remove('loading');
...
/* ================= BADGES ================= */
const BADGES=[
  { id:'first',  name:'First Blood',  desc:'Finish the tutorial', ico:()=>ICO.bolt(26),   has:()=>!!PROFILE?.tutorial_done },
  ...
  { id:'boss',   name:'Boss Slayer',  desc:'Beat a boss',         ico:()=>ICO.skull(26),  has:()=>(PROFILE?.bosses||0)>=1 },
...
function renderAvPicker(){
  ...
  AVATARS.forEach(k=>{
    ...
    b.onclick=()=>{ if(PROFILE){ PROFILE.avatar=k; saveProfile(); } paintAvatars(); renderAvPicker(); beep(660,.08); };
    w.appendChild(b);
  });
}
...
function bindRoom(code,isHost){
  ...
  $('room-hint').textContent=isHost?'Send the code to your friend — you start the duel once they join.':'Joined! Waiting for the host to start…';
  goFlow('scr-room');
  ...
  ch.on('presence',{event:'sync'},()=>{
      ...
      if(others.length){
        ...
        if(RT.host){ $('room-start').style.display='flex'; $('room-hint').textContent='Your opponent is in. Start when you are both ready.'; }
        beep(700,.1);
      } else {
...
function playVsIntro(){
  ...
  $('vs-op-av').innerHTML=mon(RT.opp?.av||'goblin',58);
...
function findMatch(){
  ...
  const lob=sb.channel('rq-lobby',{ config:{ presence:{ key:myUid() } } });
  ...
  RT.lob=lob;
  /* nobody after 12s → bot fallback (only then) */
  RT.seekT=setTimeout(()=>{
    if(!RT.seeking) return;
    RT.seeking=false;
    try{ lob.unsubscribe(); }catch(e){}
...
function queueDuel(fast){
  ...
  S.queueT=setTimeout(()=>{
    ...
    S.duel={ opp:name, oppLvl:lvl, oppElo:Math.max(100, myElo()+ri(-90,90)), target:20, my:0, op:0, sec:0, running:false, timers:[],
             tBase:S.user.avgRepSec, fatigued:false, myTs:[] };
    $('q-oname').textContent=name; $('q-olvl').textContent=lvl;
    $('q-oav').textContent=name.slice(0,2).toUpperCase();
...
function beginDuel(){
  ...
  goFlow('scr-duel');
  $('d-oname').textContent=d.opp;
  $('d-oav-big').innerHTML=mon(['goblin','troll','bone','ghoul'][ri(0,3)],56);
  resetDuelHud();
  playVsIntro();
  d.tracking=false;
...
async function startDuelTracking(d){
  ...
  if(skipBtn){ skipBtn.style.display='none'; clearTimeout(window._duelSkipT);
    window._duelSkipT=setTimeout(()=>{ if(S.duel===d && !d.tracking && d.running) skipBtn.style.display='block'; },4000); }
...
function forceDuelTapMode(){
  ...
  $('d-hint').textContent='Tap anywhere = 1 rep';
  $('d-tapzone').style.pointerEvents='auto';
  setTrackPill('d-track',null,'tap','Tap mode');
  initCam('d-video','d-fb');
  SFX.click();
}
function duelTap(){
...
function scheduleBotRep(){
  ...
  let base=d.tBase;
  if(d.op>=d.target*0.6 && !d.fatigued){ d.fatigued=true; d.fatigueMul=1+rnd(.15,.30); }
  if(d.fatigued) base*=d.fatigueMul;
  const delay=Math.max(600,(base+rnd(-0.4,0.5))*1000);
  const t=setTimeout(()=>{
...
function endDuel(win){
  ...
  saveProfile();
  showVictory();
  toast((win?'Victory! ':'Defeat. ')+(eloD>=0?'+':'')+eloD+' ELO · now '+myElo());
}
function abortDuel(){ killDuel(); nav('scr-arena'); }
...
function renderProfile(){
  if(PROFILE) $('prof-uname').textContent=PROFILE.username;
  $('p-elo').textContent=myElo();
  $('p-lvl').textContent=S.user.lvl;
  $('p-title').textContent=S.user.title;
  $('p-pet').innerHTML=S.user.pet?mon(S.user.pet,24):'';
  renderAvPicker(); paintAvatars(); renderBadges();
  renderRankCard&&0;
  const bi=$('bio-in'); if(bi && document.activeElement!==bi){ bi.value=PROFILE?.bio||''; $('bio-count').textContent=(PROFILE?.bio||'').length; }
  $('p-total').textContent=(PROFILE?.total_reps||0).toLocaleString('en-US');
  $('p-streak').textContent=S.user.streak+'🔥';
  if($('set-email')) $('set-email').textContent=ME?.email||'—';
  renderShop('shop-frames',S.shop.frames,'frame');
  renderShop('shop-titles',S.shop.titles,'title');
  renderShop('shop-pets',S.shop.pets,'pet');
  $('set-sound').classList.toggle('on',S.sound);
  [1,2,3].forEach(n=>$('sn'+n).classList.toggle('on',S.sens===n));
}
...
const POSE_CDN='https://cdn.jsdelivr.net/npm/@mediapipe/';
const EX_CFG={
  /* enterA = "down"/contracted angle, exitA = "up"/extended angle, armA = hold-here-to-start */
  'Push-ups':{ L:[11,13,15], R:[12,14,16], enterA:95,  exitA:145, armA:145, joint:'Elbow', plant:true  },
...
const PoseEngine={
  state:'idle', pose:null, cam:null, stream:null,
  ...
  async load(){
    ...
    this._loadP=(async()=>{
      try{
        await loadScript(POSE_CDN+'camera_utils/camera_utils.js');
        await loadScript(POSE_CDN+'pose/pose.js');
        this.pose=new Pose({ locateFile:f=>POSE_CDN+'pose/'+f });
  ...
  async start(videoId,canvasId,exercise,onRep,onStatus){
    ...
    try{
      ...
      console.error('[RepQuest] PoseEngine.start failed', e);
  ...
  async _startInner(video,canvas,exercise,onRep,onStatus){
    ...
    /* a previous load stuck mid-flight (e.g. tab backgrounded, stale promise) must never
       block a fresh attempt forever — if it's been "loading" too long, reset and retry clean */
    if(this.state==='loading'){
      if(!this._loadStartedAt) this._loadStartedAt=Date.now();
      if(Date.now()-this._loadStartedAt>10000){ this.state='idle'; this._loadP=null; }
    }
    if(this.state==='idle') this._loadStartedAt=Date.now();
    let ok=false;
    try{ ok = await withTimeout(this.load(), 11000, 'model load'); }
    catch(e){ this.state='failed'; ok=false; }
    if(!ok){ onStatus('tap','Tap mode — AI unavailable'); return false; }
    ...
    this.target={ video, canvas, ctx:canvas.getContext('2d'),
      ...
      phase:'READY', lastRep:0, lowVisSince:0, armed:false, extSince:0, jointName:(EX_CFG[exercise]||EX_CFG['Push-ups']).joint };
    this.cam=new Camera(video,{ onFrame:async()=>{ if(this.target) await this.pose.send({image:video}); },
      width:1280, height:720 });
    this.cam.start();
    this.target._firstResult=false;
    clearTimeout(this._resultWatchdog);
    this._resultWatchdog=setTimeout(()=>{
      if(this.target && !this.target._firstResult){
        console.warn('[RepQuest] pose model produced no results in time — falling back to tap mode');
    ...
    onStatus('ai','AI tracking ON');
    return true;
  },
  onResults(res){
    const t=this.target; if(!t) return;
    t._firstResult=true;
    this.drawPose(res.poseLandmarks,t);
    if(res.poseLandmarks) this.checkRep(res.poseLandmarks,t);
    else this.lowVis(t);
  },
  /* Maps normalized landmarks through the SAME object-fit:cover crop the <video> uses,
     so the skeleton is glued to the body instead of floating stretched over the box. */
  drawPose(lm,t){
  ...
  checkRep(lm,t){
    ...
    if(bestVis<0.4){
      t.armed=false; t.extSince=0; t.phase='READY'; t.lowVisSince=0;
      t.onStatus('warn','Step back — keep your body in frame');
      return;
    }
    ...
    if(c.plant){
      /* push-ups: hands planted AND body held in a straight plank line.
         Sitting bends the hip to ~90°, so this rules it out. */
      const shY=lm[side[0]].y, wrY=lm[side[2]].y;
      const hipI = useL ? 23 : 24, kneeI = useL ? 25 : 26;
      const canSeeTorso = (lm[hipI]?.visibility??0)>0.35 && (lm[kneeI]?.visibility??0)>0.35;
      const plankAngle = canSeeTorso ? getAngle(lm[side[0]],lm[hipI],lm[kneeI]) : 180;
      const straight = plankAngle >= 140;
      const handsDown = wrY > shY - 0.12;
      if(!handsDown || !straight){
        t.armed=false; t.extSince=0; t.phase='READY';
        t.onStatus('warn', !straight ? 'Straighten your body into a plank' : 'Get into push-up position');
        return;
      }
    }
    ...
    if(!t.armed){
      if(angle>=c.armA){
        ...
        if(now-t.extSince>500){ t.armed=true; t.onStatus('ai','IN POSITION — GO!'); beep(700,.1); }
        else t.onStatus('load','Hold steady…');
      } else { t.extSince=0; t.onStatus('load','Get to the start position'); }
      return;
    }

    t.onStatus('ai','AI · '+c.joint+' '+Math.round(angle)+'°');
    if(t.phase!=='DOWN' && angle<=c.enterA){ t.phase='DOWN'; }
    else if(t.phase==='DOWN' && angle>=c.exitA){
      if(now-t.lastRep>550){ t.lastRep=now; t.phase='READY'; t.onRep(); }
    }
  },
  stop(){
    const t=this.target;
    this.target=null;
    try{ this.cam?.stop(); }catch(e){}
    this.cam=null;
...
function authTab(m){
  ...
  $('f-username').style.display=m==='signup'?'block':'none';
...
async function doAuth(){
  ...
  const uname=$('in-username').value.trim();
  if(!email||!pass){ $('auth-err').textContent='Fill in email and password.'; return; }
  if(authMode==='signup' && uname.length<3){ $('auth-err').textContent='Username needs 3+ characters.'; return; }
...
async function setNewPassword(){
  ...
  try{
    ...
    if(error) throw error;
    SFX.win(); confetti();
    openModal('<div class="em">✅</div><h3 class="hdr">Password updated!</h3><p>You\'re signed in with your new password.</p><button class="btn b-purple" onclick="closeModal();afterPasswordReset()">CONTINUE</button>');
  }catch(e){
...
async function loadProfile(){
  if(!sb||!ME) return;
  const DEFAULTS={ id:ME.id, username:ME.user_metadata?.username||'Fighter', level:1, xp:0, coins:200,
    ...
    difficulty:'easy', story_seen:'', offer_started:null };
  let data=null;
  try{ ({ data } = await sb.from('profiles').select('*').eq('id',ME.id).maybeSingle()); }
  catch(e){ console.error('[RepQuest] profile load failed:', e.message||e); }

  const local=loadLocalProfile();
  if(data){
    /* DB row wins, but fill any column the DB doesn't have yet from the local backup */
    PROFILE={ ...DEFAULTS, ...(local||{}), ...data };
    Object.keys(DEFAULTS).forEach(k=>{
      if(data[k]===undefined && local && local[k]!==undefined) PROFILE[k]=local[k];
    });
    /* a truthy local tutorial flag must never be lost to a stale/failed cloud write */
    if(local?.tutorial_done) PROFILE.tutorial_done=true;
    if(local?.avatar_url && !data.avatar_url) PROFILE.avatar_url=local.avatar_url;
  } else {
    PROFILE={ ...DEFAULTS, ...(local||{}) };
    await pushProfile();
  }
  cacheProfileLocally();
  /* apply to game state */
...
const PROFILE_COLS=['id','username','level','xp','coins','streak','pr','total_reps','tutorial_done',
  'membership','elo','bio','avatar','avatar_url','duel_wins','bosses','difficulty','story_seen','offer_started'];
let badCols=new Set();        /* columns the DB rejected — skipped on later saves */
let warnedSave=false;

function localKey(){ return 'rq_profile_'+(ME?.id||'anon'); }
...
async function pushProfile(attempt){
  ...
  try{
    ...
    if(col && PROFILE_COLS.includes(col) && !badCols.has(col) && attempt<PROFILE_COLS.length){
      badCols.add(col);
      console.warn('[RepQuest] DB is missing column "'+col+'" — saving without it. Run the latest SQL migration.');
      return pushProfile(attempt+1);
    }
    console.error('[RepQuest] profile save failed:', msg);
    if(!warnedSave){
      warnedSave=true;
...
function confirmLogout(){
  SFX.click();
  openModal('<div class="em">🚪</div><h3 class="hdr">Sign out?</h3>'+
...
function obNext(from){
  if(from===1){
    ...
    $('ob-greet').textContent='The Arena Calls, '+n;
    obStep(2);
...
function enterApp(){
  ...
  setTimeout(()=>toast(greetLine()+', '+(PROFILE?.username||'Fighter')+'!'),500);
}
...
function startTutorial(){
  ...
  if(PROFILE && !(PROFILE.story_seen||'').includes('intro')){
    ...
    showStoryModal('Your first fight',
      'A goblin blocks the cave mouth. It has three points of life.<br><br>'+
      'Three clean push-ups will end it. Get down, get straight, and start swinging.',
      'FIGHT', ()=>beginTutorialFight());
    return;
  }
  beginTutorialFight();
}
function beginTutorialFight(){
  ...
  makeBattle({ kind:'tutorial', exercise:'Push-ups',
    waves:[{name:'Training Goblin',sprite:'goblin',hp:3}], modeName:'Tutorial' });
}
...
/* ================= LOADING SCREEN ================= */
const LD_TIPS=[
  'Every clean rep is one point of damage.',
  'Stall too long and your stamina drains.',
  '14% of your reps land as CRITS for triple damage.',
  'Harder difficulty means far more XP and coins.',
  'Beat a friend in a live duel to climb the ELO ladder.',
  'Bosses wait at the end of every 6th episode.',
];
let ldVal=0, ldTimer=null, ldTipTimer=null;
function ldSet(p,msg){
  ldVal=Math.max(ldVal,Math.min(100,p));
  $('ld-fill').style.width=ldVal+'%';
  $('ld-pct').textContent=Math.round(ldVal)+'%';
  if(msg) $('ld-tip').textContent=msg;
}
function ldStart(){
  let i=Math.floor(Math.random()*LD_TIPS.length);
  $('ld-tip').textContent=LD_TIPS[i];
  ldTipTimer=setInterval(()=>{
    i=(i+1)%LD_TIPS.length;
    const t=$('ld-tip'); t.style.animation='none'; void t.offsetWidth; t.style.animation='';
    t.textContent=LD_TIPS[i];
  },2600);
  ldTimer=setInterval(()=>{ if(ldVal<88) ldSet(ldVal+Math.random()*7); },260);
}
function ldDone(){
  clearInterval(ldTimer); clearInterval(ldTipTimer);
  ldVal=Math.max(ldVal,0);
  ldSet(100,'Ready.');
...
async function boot(){
  ...
  const failsafe=setTimeout(()=>{
    console.warn('[RepQuest] boot failsafe fired');
    ldDone();
    if(!document.querySelector('.screen.active') || $('scr-splash').classList.contains('active')) goFlow('scr-auth');
  }, 9000);
  try{
    goFlow('scr-splash');
    ...
    let session=null;
    try{ session=(await sb?.auth.getSession())?.data?.session||null; }catch(e){ console.error('[RepQuest] session error',e); }
    ...
    if(session){
      ...
      try{ await loadProfile(); }catch(e){ console.error('[RepQuest] loadProfile failed',e); }
      try{ enterApp(); }catch(e){ console.error('[RepQuest] enterApp failed',e); goFlow('scr-auth'); }
    } else {
      goFlow('scr-auth');
    }
  }catch(e){
    console.error('[RepQuest] boot failed',e);
    goFlow('scr-auth');
  }finally{
